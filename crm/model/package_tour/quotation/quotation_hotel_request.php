...
class quotation_hotel_request{
    ...
    public function send_common(){
        ...
        $hotel_status_arr1 = ($_POST['hotel_status_arr'] != '') ? json_encode($_POST['hotel_status_arr']) : [];
        $hotel_entry_arr = $_POST['hotel_entry_arr'];
        ...
        for($i=0;$i<sizeof($hotel_entry_arr);$i++){
            ...
            // For optional hotels
            $option_hotel_arr = ($hotel_status_arr[$i]->option_hotel_arr != '') ? $hotel_status_arr[$i]->option_hotel_arr : [];
    ...
    function supplier_response_toAdmin($request_id){
        ...
        $availability_details1 = ($sq_hotel_qout['availability']!='') ? $sq_hotel_qout['availability'] : [];
        $availability_details = json_decode($availability_details1);
        if($hotel_flag != ''){
            ...
            $spec = ($availability_details->spec != '') ? $availability_details->spec : 'NA';
        }

        $emp_id = $availability_details->emp_id;
        $sq_city = mysqli_fetch_assoc(mysqlQuery("select * from city_master where city_id='$sq_hotel_qout[city_name]'"));
