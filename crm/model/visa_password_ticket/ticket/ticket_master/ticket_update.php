...

{
	...
	for($i=0; $i<sizeof($first_name_arr); $i++){
		...
		if($checked_arr[$i] != 'true'){
			...
			$trip_entry_id_arr = $trip_details_arr3[0]->trip_entry_id_arr;
			$trip_data_check_arr = $trip_details_arr3[0]->trip_data_check_arr;
			$cancel_status_arr = $trip_details_arr3[0]->cancel_status_arr;
			...
			for($j=0; $j<sizeof($departure_datetime_arr); $j++){
				if($trip_data_check_arr[$j]){
					...
					if($entry_id_arr[$i]=="" && $cancel_status_arr[$j] != "Cancel"){
						...
						$sq_count = mysqli_num_rows(mysqlQuery("select * from ticket_trip_entries where airlin_pnr='$airlin_pnr_arr[$j]' and airlin_pnr!=''"));
