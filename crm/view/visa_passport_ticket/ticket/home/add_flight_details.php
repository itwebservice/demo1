...
$count = $_POST['count'];
$pass_entry_id = $_POST['entry_id'];

$type_of_tour = $flight_details[0]->type_of_tour;
$departure_datetime_arr = $flight_details[0]->departure_datetime_arr;
$loop_items = (($departure_datetime_arr) != '') ? sizeof($departure_datetime_arr) : '1';
                ...
                <form id="frm_flight_details">
                    ...
                    <div class="dynform-wrap" id="div_dynamic_ticket_info" data-counter="<?= $loop_items ?>">
						...
						for($i = 0; $i < $loop_items; $i++){
							...
							$entry_id = $flight_details[0]->trip_entry_id_arr[$i];
							$css = '';
							if($type == 'update'){
								$button_name = 'Update';
							}
							$date = ($departure_datetime_arr[$i]!='') ? $departure_datetime_arr[$i] : date('d-m-Y H:i');
							$sq_entry = mysqli_fetch_assoc(mysqlQuery("select * from ticket_trip_entries where entry_id='$entry_id'"));
							$bg = ($sq_entry['status'] == 'Cancel') ? 'background-color:#f2dede!important;' : '';
							$disbaled = ($sq_entry['status'] == 'Cancel') ? 'disabled' : '';
							$css =  'style="margin-top :0px !important;'.$bg.'"';
							$check_status = ($flight_details[0]->trip_data_check_arr[$i]) ? 'checked' : '';
							...
							<div class="dynform-item" for="chk_tickett<?= $sq_trip_entries_count ?>" <?=$css?>>
								<div class="row">
									...
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="arrival_datetime-<?= $sq_trip_entries_count ?>" name="arrival_datetime" class="app_datetimepicker arrival_datetime" placeholder="*Arrival Date-Time" onchange="validate_validDatetimeFlight(this.id)" title="Arrival Date-Time" value="<?php echo $flight_details[0]->arrival_datetime_arr[$i] ?>" data-dyn-valid="required">
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="flight_duration-<?= $sq_trip_entries_count ?>" name="flight_duration" placeholder="Flight Duration" title="Flight Duration" value="<?= $flight_details[0]->flight_duration_arr[$i] ?>" >
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
									...
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="sub_category-<?= $sq_trip_entries_count ?>" name="sub_category"  placeholder="Sub Category" title="Sub Category" value="<?= $flight_details[0]->sub_category_arr[$i] ?>" >
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="flight_no-<?= $sq_trip_entries_count ?>" style="text-transform: uppercase;" name="flight_no" onchange="validate_specialChar(this.id)" placeholder="Flight No" title="Flight No" data-dyn-valid="" value="<?= $flight_details[0]->flight_no_arr[$i] ?>">
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="airlin_pnr-<?= $sq_trip_entries_count ?>" style="text-transform: uppercase;" onchange=" validate_specialChar(this.id)" name="airlin_pnr" placeholder="GDS PNR" title="GDS PNR" data-dyn-valid="" value="<?= $flight_details[0]->airlin_pnr_arr[$i] ?>">
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<?php
										$city_id = $flight_details[0]->from_city_id_arr[$i];
										$sq_city = mysqli_fetch_assoc(mysqlQuery("select city_name from city_master where city_id='$city_id'"));
										?>
										<input id="airpf-<?= $sq_trip_entries_count ?>" name="airpf" title="Enter Departure Airport" data-toggle="tooltip" class="form-control autocomplete airpf" placeholder="*Enter Departure Airport" data-dyn-valid="required" value="<?php echo ($sq_city['city_name']) ? $sq_city['city_name']." - ".$flight_details[0]->departure_city_arr[$i] : ''; ?>">
										<input type="hidden" name="from_city" id="from_city-<?= $sq_trip_entries_count ?>" data-dyn-valid="required" value="<?= $flight_details[0]->from_city_id_arr[$i] ?>"/>
										<input type="hidden" name="departure_city" id="departure_city-<?= $sq_trip_entries_count ?>" data-dyn-valid="required" value="<?= $flight_details[0]->departure_city_arr[$i] ?>">
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="dterm-<?= $sq_trip_entries_count ?>" name="dterm" onchange="validate_specialChar(this.id)" placeholder="Departure Terminal" title="Departure Terminal" data-dyn-valid="" value="<?= $flight_details[0]->departure_terminal_arr[$i] ?>">
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10 ">
										<?php
										$city_id = $flight_details[0]->to_city_id_arr[$i];
										$sq_city = mysqli_fetch_assoc(mysqlQuery("select city_name from city_master where city_id='$city_id'"));
										?>
										<input id="airpt-<?= $sq_trip_entries_count ?>" name="airpt" class="form-control autocomplete airpt" title="Enter Arrival Airport" data-toggle="tooltip" placeholder="*Enter Arrival Airport" data-dyn-valid="required" value="<?php echo ($sq_city['city_name']) ? $sq_city['city_name']." - ".$flight_details[0]->arrival_city_arr[$i] : ''; ?>" >
										<input type="hidden" name="to_city" id="to_city-<?= $sq_trip_entries_count ?>" data-dyn-valid="required" value="<?= $flight_details[0]->to_city_id_arr[$i] ?>"/>
										<input type="hidden" name="arrival_city" id="arrival_city-<?= $sq_trip_entries_count ?>" data-dyn-valid="required" value="<?php echo ($flight_details[0]->arrival_city_arr[$i] != '')? $flight_details[0]->arrival_city_arr[$i]: '';?>">
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="aterm-<?= $sq_trip_entries_count ?>" name="aterm" onchange="validate_specialChar(this.id)" placeholder="Arrival Terminal" title="Arrival Terminal" data-dyn-valid="" value="<?= $flight_details[0]->arrival_terminal_arr[$i] ?>">
									</div>
									...
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="no_of_pieces-<?= $sq_trip_entries_count ?>" name="no_of_pieces"  placeholder="No of pieces" title="No of pieces" value="<?= $flight_details[0]->no_of_pieces_arr[$i] ?>" >
									</div>
									<div class="col-md-3 col-sm-12 col-xs-12 mg_bt_10">
										<textarea name="special_note" id="special_note-<?= $sq_trip_entries_count ?>" onchange="validate_address(this.id)" rows="1" placeholder="Special Note" title="Special Note" data-dyn-valid=""><?= $flight_details[0]->special_note_arr[$i] ?></textarea>
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="aircraft_type-<?= $sq_trip_entries_count ?>" name="aircraft_type"  placeholder="Aircraft Type" title="Aircraft Type" value="<?= $flight_details[0]->aircraft_type_arr[$i] ?>" >
									</div>
								</div>	
								<div class="row">
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="operating_carrier-<?= $sq_trip_entries_count ?>" name="operating_carrier" placeholder="Operated By" title="Operated By" value="<?= $flight_details[0]->operating_carrier_arr[$i] ?>" >
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="text" id="frequent_flyer-<?= $sq_trip_entries_count ?>" name="frequent_flyer" placeholder="Frequent Flyer" title="Frequent Flyer" value="<?= $flight_details[0]->frequent_flyer_arr[$i] ?>" >
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<select name="ticket_status" id="ticket_status-<?= $sq_trip_entries_count ?>" title="Status of ticket" >
											<?php if($flight_details[0]->ticket_status_arr[$i]!=''){?><option value="<?= $flight_details[0]->ticket_status_arr[$i] ?>"><?= $flight_details[0]->ticket_status_arr[$i] ?></option><?php } ?>
									...
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<input type="number" id="basic_fare-<?= $sq_trip_entries_count ?>" name="basic_fare" placeholder="Basic Fare" title="Basic Fare"value="<?= $flight_details[0]->basic_fare_arr[$i] ?>" >
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10">
										<select name="refund_type" id="refund_type-<?= $sq_trip_entries_count ?>" title="Refund Type" data-dyn-valid="required" >
										<?php if($flight_details[0]->refund_type_arr[$i]!=''){?><option value="<?= $flight_details[0]->refund_type_arr[$i] ?>"><?= $flight_details[0]->refund_type_arr[$i] ?></option><?php } ?>
									...
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10 hidden">
										...
										value="<?= $flight_details[0]->trip_entry_id_arr[$i] ?>"/>
									</div>
									<div class="col-md-3 col-sm-4 col-xs-12 mg_bt_10 hidden">
