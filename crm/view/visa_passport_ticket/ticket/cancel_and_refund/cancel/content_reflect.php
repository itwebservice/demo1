...
$cancel_estimate = ($sq_ticket_info['cancel_estimate']!='') ? json_decode($sq_ticket_info['cancel_estimate'])[0] : '';
?>
...
<form id="frm_refund">
    ...
    <div class="row mg_tp_20">
        <div class="col-sm-3 col-xs-12 mg_bt_10">
            ...
            <input type="number" id="basic_cost" name="basic_cost" placeholder="Basic Amount" title="Basic Amount"  onchange="calculate_total_amount(this.id);" value="<?= floatval($cancel_estimate->basic_cost) ?>">
        </div>
        <div class="col-sm-3 col-xs-12 mg_bt_10">
            <small>YQ Tax</small>
            <input type="number" id="yq_tax" name="yq_tax" placeholder="YQ Tax" title="YQ Tax" onchange="calculate_total_amount(this.id);" value="<?= floatval($cancel_estimate->yq_tax) ?>">
        </div>
        <div class="col-sm-3 col-xs-12 mg_bt_10">
            <small>Other Taxes</small>
            <input type="number" id="other_taxes" name="other_taxes" placeholder="Other Taxes" title="Other Taxes" onchange="calculate_total_amount(this.id);" value="<?= floatval($cancel_estimate->other_taxes) ?>" >
        </div>
        <div class="col-sm-3 col-xs-12 mg_bt_10">
            <small id="discount_show">Discount</small>
            <input type="number" id="discount" name="discount" placeholder="Discount" title="Discount" onchange="calculate_total_amount(this.id);" value="<?= floatval($cancel_estimate->discount) ?>" >
        </div>
        <div class="col-sm-3 col-xs-12 mg_bt_10">
            <small id="service_show">Service Charge</small>
            <input type="number" id="service_charge" name="service_charge" placeholder="Service Charge" title="Service Charge" onchange="calculate_total_amount(this.id);" value="<?= floatval($cancel_estimate->service_charge) ?>" >
        </div>
        <div class="col-sm-3 col-xs-12 mg_bt_10">
            <small>Tax Amount</small>
            <input type="number" id="service_tax_subtotal" name="service_tax_subtotal" placeholder="Tax Amount" title="Tax Amount" onchange="calculate_total_amount(this.id);" value="<?= floatval($cancel_estimate->service_tax_subtotal) ?>">
        </div>
        <div class="col-sm-3 col-xs-12 mg_bt_10">
            <small id="markup_show">Markup</small>
            <input type="text" id="markup" name="markup" placeholder="Markup" title="Markup" onchange="calculate_total_amount(this.id);" value="<?= floatval($cancel_estimate->markup) ?>" >
        </div>
        <div class="col-sm-3 col-xs-12 mg_bt_10">
            <small>Tax on Markup</small>
            <input type="text" id="service_tax_markup" name="service_tax_markup" placeholder="Tax on Markup" title="Tax on Markup" onchange="calculate_total_amount(this.id);" value="<?= floatval($cancel_estimate->service_tax_markup) ?>">
        </div>
        <div class="col-md-3 col-sm-3 col-xs-12 mg_bt_10">
            <small>TDS</small>
            <input type="number" id="tds" name="tds" placeholder="TDS" title="TDS" onchange="calculate_total_amount(this.id);validate_balance(this.id)" value="<?= floatval($cancel_estimate->tds) ?>">
        </div>		
        <div class="col-md-3 col-sm-3 col-xs-12 mg_bt_10">
            <small>Roundoff</small>
            <input type="text" name="roundoff" id="roundoff" class="text-right" placeholder="Round Off" title="RoundOff" value="<?= floatval($cancel_estimate->roundoff) ?>" readonly>
        </div>	
        <div class="col-md-3 col-sm-3 col-xs-12 mg_bt_10_sm_xs">
            <small>Net Total</small>
            <input type="text" name="ticket_total_cost" id="ticket_total_cost" class="amount_feild_highlight text-right" placeholder="Net Total" title="Net Total" readonly value="<?= floatval($cancel_estimate->ticket_total_cost) ?>">
        </div>
    </div>
	<div class="row mg_tp_30 mg_bt_30">
		<div class="col-md-3 col-md-offset-3 col-sm-6 col-xs-12 mg_bt_10_xs">
            <small class="text-left">Cancellation Charges</small>
                ...
                callback: function(data1){
                    ...
                    if(data1=="yes"){
                        ...
                        $.ajax({
                            ...
                            data:{ ticket_id : ticket_id,cancel_amount : cancel_amount, total_refund_amount : total_refund_amount,estimate_arr:estimate_arr},
                            success:function(result){
                            msg_alert(result);
                            content_reflect();
