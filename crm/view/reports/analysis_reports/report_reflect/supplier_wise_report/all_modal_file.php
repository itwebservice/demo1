...
{
$qryPackage = mysqlQuery("select customer_master.customer_id,customer_master.first_name,customer_master.last_name,vendor_estimate.net_total,vendor_estimate.basic_cost
from vendor_estimate inner join package_tour_booking_master on vendor_estimate.vendor_type_id = package_tour_booking_master.booking_id 
...
estimate_type='Package Tour'" . $dateqry) or die('error');
}
//group
if($estimateType == 'Group Tour')
{
$qryGroup = mysqlQuery("select customer_master.customer_id,customer_master.first_name,customer_master.last_name,vendor_estimate.net_total,vendor_estimate.basic_cost from vendor_estimate left join tourwise_traveler_details on vendor_estimate.vendor_type_id = tourwise_traveler_details.id 
inner join customer_master on tourwise_traveler_details.customer_id=customer_master.customer_id
where vendor_estimate.vendor_type_id='$vendorTypeId' and vendor_estimate.vendor_type='$vendorType' and 
estimate_type='Group Tour'" . $dateqry) or die('error');
}

//gen
if ($estimateType != 'Package Tour'  && $estimateType != 'Group Tour'  && $vendorType != 'Other Vendor' ) {
    ...
    if ($vendorType == 'Car Rental Vendor') {
        $genQry .= " inner join car_rental_booking on vendor_estimate.vendor_type_id = car_rental_booking.booking_id 
        inner join customer_master on car_rental_booking.booking_id=customer_master.customer_id";
    }
    if ($vendorType == 'Train Ticket Vendor') {
        $genQry .= " inner join train_ticket_master on vendor_estimate.vendor_type_id = train_ticket_master.train_ticket_id 
        inner join customer_master on train_ticket_master.customer_id=customer_master.customer_id";
    }
    if ($vendorType == 'Visa Vendor') {
        $genQry .= " inner join visa_vendor on vendor_estimate.vendor_type_id = visa_vendor.vendor_id 
        inner join customer_master on visa_vendor.vendor_id=customer_master.customer_id";
    }
    if ($vendorType == 'Ticket Vendor') {
        $genQry .= " inner join ticket_master on vendor_estimate.vendor_type_id = ticket_master.ticket_id 
        inner join customer_master on ticket_master.customer_id=customer_master.customer_id";
    }
    if ($vendorType == 'Hotel Vendor') {
        $genQry .= " inner join hotel_booking_master on hotel_booking_master.booking_id=vendor_estimate.estimate_type_id
        inner join customer_master on hotel_booking_master.customer_id=customer_master.customer_id";
    }
    if ($vendorType == 'Excursion Vendor') 
    {
        $genQry .= " inner join excursion_master on vendor_estimate.vendor_type_id = excursion_master.exc_id 
        inner join customer_master on excursion_master.customer_id=customer_master.customer_id";
    }
    if($estimateType == "Train Ticket Booking" && $vendorType != 'DMC Vendor')
    {
        $genQry .= " inner join train_ticket_master on vendor_estimate.vendor_type_id = train_ticket_master.train_ticket_id 
        inner join customer_master on train_ticket_master.customer_id=customer_master.customer_id";

    }
    // if($estimateType == "Hotel Booking" && $vendorType != 'DMC Vendor')
    // {
    //     $genQry .= " inner join hotel_booking_master on hotel_booking_master.booking_id=vendor_estimate.estimate_type_id
    //     inner join customer_master on hotel_booking_master.customer_id=customer_master.customer_id";
    // }
    ...
    $genQry .= " where vendor_estimate.vendor_type_id='$vendorTypeId' and vendor_estimate.vendor_type='$vendorType'" . $dateqry;
                ...
                <table class="table">
                    ...
                    ?>
                                <tr>
                                    ...
                                    <td><?php 
                                    $totalAll += (int)$db['basic_cost'];
                                    echo $db['basic_cost']; ?> </td>
                                </tr>
                            <?php
                            }
                        }
