<?php
// for query:  mysqlQuery($query1);   
// for rows:  mysqli_num_rows($res); if condition
// for fetch: mysqli_fetch_assoc($res); while condition
...
function transport($dateQry)
{
    global $array_s;
    global $count;
    $qrytransport = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total) as netTotal,count(*) as `totalPurchase`,
    ...
    where vendor_estimate.vendor_type="Transport Vendor"' . $dateQry . ' GROUP BY vendor_estimate.vendor_type_id';
...
function trainTicket($dateQry)
{
    global $array_s;
    global $count;
    $qrytrain = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total) as netTotal,count(*) as `totalPurchase`,
    vendor_estimate.vendor_type_id as `vendorTypeId`, vendor_estimate.vendor_type as `vendorType`, vendor_estimate.estimate_type_id as `estimateTypeId`, vendor_estimate.estimate_type as `estimateType`,
    ...
    where vendor_estimate.vendor_type="Train Ticket Vendor"' . $dateQry . ' GROUP BY vendor_estimate.vendor_type_id';
...
function ticketVendor($dateQry)
{
    global $array_s;
    global $count;
    $qryt = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total) as netTotal,count(*) as `totalPurchase`,
    ...
    where vendor_estimate.vendor_type="Ticket Vendor"' . $dateQry . ' GROUP BY vendor_estimate.vendor_type_id';
...
function cruiseMsater($dateQry)
{
    global $array_s;
    global $count;
    $qryc = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total) as netTotal,count(*) as `totalPurchase`,
    ...
    where vendor_estimate.vendor_type="Cruise Vendor"' . $dateQry . ' GROUP BY vendor_estimate.vendor_type_id';
...
function siteSeeing($dateQry)
{
    global $array_s;
    global $count;
    $qrys = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total) as netTotal,count(*) as `totalPurchase`,
    ...
    where vendor_estimate.vendor_type="Excursion Vendor"' . $dateQry . ' GROUP BY vendor_estimate.vendor_type_id';
...
function dmcMaster($dateQry)
{
    global $array_s;
    global $count;
    $qryd = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total) as netTotal,count(*) as `totalPurchase`,
    ...
    where vendor_estimate.vendor_type="Dmc Vendor"' . $dateQry . ' GROUP BY vendor_estimate.vendor_type_id';
...
function visaVendor($dateQry)
{
    global $array_s;
    global $count;
    $qryv = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total) as netTotal,count(*) as `totalPurchase`,
    ...
    where vendor_estimate.vendor_type="Visa Vendor"' . $dateQry . ' GROUP BY vendor_estimate.vendor_type_id';
...
function insuranceVendor($dateQry)
{
    global $array_s;
    global $count;
    $qryi = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total) as netTotal,count(*) as `totalPurchase`,
    ...
    where vendor_estimate.vendor_type="Insurance Vendor"' . $dateQry . ' GROUP BY vendor_estimate.vendor_type_id';
...
//     global $array_s;
//     global $count;
//     $qryo = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total) as netTotal,count(*) as `totalPurchase`, 
...
function carrentalVendor($dateQry)
{
    global $array_s;
    global $count;
    $qryr = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total)as netTotal,count(*) as `totalPurchase`,
    ...
    from vendor_estimate inner join car_rental_vendor on car_rental_vendor.vendor_id=vendor_estimate.vendor_type_id
     where vendor_estimate.vendor_type="Car Rental Vendor"' . $dateQry . ' GROUP BY vendor_estimate.vendor_type_id';
...
function hotelVendor($dateQry)
{
    global $array_s;
    global $count;
    $qryh = 'select SUM(vendor_estimate.basic_cost)as basicCost,SUM(vendor_estimate.net_total)as netTotal,count(*) as `totalPurchase`,
    ...
    where vendor_estimate.vendor_type="Hotel Vendor"' . $dateQry . ' GROUP BY vendor_estimate.vendor_type_id';
