                ...
                <thead>
                    ...
                    <tbody>
                        ...
                        if($role == 'Admin'){
                            ...
                            $sq_booking = mysqlQuery("select customer_id,cart_checkout_data,created_at,coupon_code,status,booking_id from b2b_booking_master where 1");
                            while($row_booking = mysqli_fetch_assoc($sq_booking)){
                                $sq_regs = mysqli_fetch_assoc(mysqlQuery("select register_id,whatsapp_no,cp_first_name,cp_last_name,company_name from b2b_registration where customer_id='$row_booking[customer_id]' and approval_status='Approved'"));
                                $sq_count = mysqli_num_rows(mysqlQuery("select register_id from b2b_creditlimit_master where register_id='$sq_regs[register_id]' and approval_status='Approved'"));
                                if($sq_count != 0){
                                    $hotel_total = 0;
                                    $transfer_total = 0;
                                    $activity_total = 0;
