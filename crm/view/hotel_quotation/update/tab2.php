        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-12 col-sm-8 col-xs-12 no-pad table-responsive">
                    <table style="width: 100%" id="hotel_quotation_update" name="hotel_quotation_update"
                        class="table table-bordered table-hover table-striped no-marg pd_bt_51 mg_bt_0">
                        <?php if ($hotelDetails == '') { ?>
                        <tr>
                            <td style="width: 50px;"><input class="css-checkbox mg_bt_10" id="chk_program-u_1"
                                    type="checkbox" checked><label class="css-label" for="chk_program-u_1"> <label></td>
                            ...
                            <td><select id="city_name-u_1" name="city_name-u_1" class="city_master_dropdown"
                                    style="width:160px" onchange="hotel_name_list_load(this.id);"
                                    title="Select City Name">
                                </select>
                            </td>

                            <td><select id="hotel_name-u_1" name="hotel_name-u_1" onchange="hotel_type_load(this.id);"
                                    style="width:160px" title="Select Hotel Name">
                                    <option value="">Hotel Name</option>
                                </select>
                            </td>

                            <td><select name="room_cat-u_1" id="room_cat-u_1" style="width:145px;" title="Room Category"
                                    class="form-control app_select2" onchange=""><?php get_room_category_dropdown(); ?>
                                </select>
                            </td>

                            <td><select name="meal_plan-u_1" id="meal_plan-u_1" style="width:145px;" title="Meal Plan"
                                    class="form-control app_select2"><?php get_mealplan_dropdown(); ?>
                                </select>
                            </td>

                            <td><input type="text" style="width:150px;" class="app_datepicker" id="check_in-u_1"
                                    name="check_in-u_1" placeholder="Check-In Date" title="Check-In Date"
                                    onchange="get_auto_to_date(this.id);" value="<?= $values['checkin'] ?>">
                            </td>

                            <td><input type="text" style="width:150px;" class="app_datepicker" id="check_out-u_1"
                                    name="check_out-u_1" placeholder="Check-Out Date" title="Check-Out Date"
                                    onchange="calculate_total_nights(this.id);validate_validDates(this.id);"
                                    value="<?= $values['checkout'] ?>">
                            </td>

                            <td><input type="text" id="hotel_type-u_1" name="hotel_type-1" placeholder="Hotel Category"
                                    title="Hotel Category" style="width:150px" value="<?= $values['hotel_type'] ?>"
                                    readonly>
                            </td>

                            <td><input type="text" id="hotel_stay_days-u_1" title="Total Nights"
                                    name="hotel_stay_days-u_1" placeholder="Total Nights"
                                    value="<?= $values['hotel_stay_days'] ?>" onchange="validate_balance(this.id);"
                                    style="width:150px;" readonly></td>

                            <td><input type="text" id="no_of_rooms-u_1" title="Total Rooms" name="no_of_rooms-u_1"
                                    placeholder="Total Rooms" value="<?= $values['total_rooms'] ?>"
                                    onchange="validate_balance(this.id);" style="width:110px"></td>

                            <td><input type="text" id="extra_bed-u_1" name="extra_bed-u_1" title="Extra Bed"
                                    placeholder="Extra Bed" onchange="validate_balance(this.id);" style="width:100px"
                                    value="<?= $values['extra_bed'] ?>"></td>

                            <td class="hidden"><input type="hidden" id="hotel_cost-u_1" name="hotel_cost-u_1"
                                    placeholder="Hotel Cost" title="Hotel Cost" onchange="validate_balance(this.id)"
                                    style="width:100px;" value="<?= $values['hotel_cost'] ?>"></td>
                        </tr>
                        <?php
						} else {
							$count = 1;
