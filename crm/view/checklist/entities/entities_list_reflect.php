            ...
            <table class="table table-bordered" id="checklist_entities_tbl">
                ...
                <tbody>
					...
					while ($row_entity = mysqli_fetch_assoc($sq_entity)) {
						...
						$sq_pckg_tour = mysqli_fetch_assoc(mysqlQuery("select * from package_tour_booking_master where booking_id='$row_entity[booking_id]' and delete_status='0'"));
                    ...
                    <tr>
                        <td><?= ++$count ?></td>
                        <td><?= $entity_for1 ?></td>
                        <td><button class="btn btn-info btn-sm" onclick="view_modal(<?= $row_entity['entity_id']  ?>)"
                                title="View Checklist"><i class="fa fa-eye"></i></button>
                            <button class="btn btn-info btn-sm" onclick="update_modal(<?= $row_entity['entity_id'] ?>)"
                                title="Edit Checklist"><i class="fa fa-pencil-square-o"></i></button>
                        </td>
                    </tr>
                    <?php
					}
